...
function getWeekNumberByDate(date) {
  ...
  let firstDayOfWeek = new Date(year, 0, 1).getDay();

  if (firstDayOfWeek === 0) {
    firstDayOfWeek = 7;
  }

  const daysInFirstWeek = 7 - firstDayOfWeek + 1;
  ...
  const daysInCurrentMonth = new Date(date).getDate();
